...
const StylistCard = ({item}) => {
 ...
 <Image source={{uri:item?.image}}  style={{height:'100%', width:'100%', backgroundColor:'white'}} />
            ...
            <View style={{flex:1, paddingHorizontal:12, gap:6}}>
                ...
                <Text style={{fontFamily:'light', color:'gray', fontSize:12}}>{`Staff (${item?.experience} years of experience)`}</Text>

                <Text style={{fontFamily:'light', color:'black', fontSize:14}}>{item?.profession}</Text>
        ...
        <View style={{gap:8, paddingHorizontal:8}}>
            ...
            <NailLongButton color='purple' onPress={()=> {}} label='Reserve Staff' />
